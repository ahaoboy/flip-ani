...
export function getImgList(size = 2) {
  return sampleSize(IMG_LIST, size)
}
...
export function createSrcRectMap(imgs) {
  return imgs.reduce((prev, img) => {
    const rect = img.getBoundingClientRect()
    console.log('prev,img', img, rect)
    const {left, top, height, width} = rect
    prev[img.src] = {left, top, img, height, width}
    return prev
  }, {})
}
